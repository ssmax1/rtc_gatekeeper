...
int diagIndex = 0;   // 0=outcome,1=success,2=fail,3=temp,4=close,5=resets,6=exit
enum PulseOutcome {
  OUT_NONE,
  OUT_OPEN_BREAK,
...
int servoOpen = 90;     // adjust for your mechanism
...
static unsigned long lastAwakeWdtCheck = 0;
...
void setup() {
  ...
  EEPROM.get(EEPROM_RESETCOUNT_ADDR, watchdogResetCount);
  if (watchdogResetCount < 0 || watchdogResetCount > 9999) {
...
void showWelcomeAfterLongSleepIfNeeded() {
  ...
  if (currentTempC > 4 && slept.totalseconds() > 300 ) {
    byte heartBuf[8];
    memcpy_P(heartBuf, heart, 8);
    lcd.createChar(0, heartBuf);
    lcd.clear();
...
// --- Updated Button Handlers (Diagnostics & WDT Crash Confirmation) ---

void handleUp() {
  switch (menuState) {
    case HOME: break;
    case DIAGNOSTICS:
      diagIndex = (diagIndex + 1) % 8;
      break;
    case MODE_SELECT:
    ...
    case OPTIONS:
      if (!optionsEditMode) {
        optionsFieldIndex = (optionsFieldIndex + 1) % 8;
      } else {
...
void handleDown() {
  switch (menuState) {
    case HOME: break;
    case DIAGNOSTICS:
      diagIndex = (diagIndex + 7) % 8;
      break;
    case MODE_SELECT:
      fieldIndex = (fieldIndex + 2) % 3;
      break;
    case OPTIONS:
      if (!optionsEditMode) {
        optionsFieldIndex = (optionsFieldIndex + 7) % 8;
      } else {
        if (optionsFieldIndex == 0) {
          sleepTimeoutMs -= 1000;
...
void handleRight() {
  switch (menuState) {
    ...
    case CONFIRM_WDT_CRASH:
      lcd.clear();
      if (cancelChoice) {
        lcd.setCursor(0,0); lcd.print(F("Crash Test Start"));
        lcd.setCursor(0,1); lcd.print(F("Triggering WDT.."));
        delay(1500);

        // Enter infinite loop to trip WDT
        while(1);
      } else {
        lcd.setCursor(0,0); lcd.print(F("Test Cancelled"));
        lcd.setCursor(0,1); lcd.print(F("Returning..."));
        lastMessageStart = millis();
        showingMessage = true;
...
void refreshLCD() {
  ...
  switch (menuState) {
    ...
    case MODE_SELECT: {
      ...
      else if (fieldIndex == 1) lcd.print(F("> Options"));
      else if (fieldIndex == 2) lcd.print(F("> Set Clock"));
      break;
    }
    ...
    case DIAGNOSTICS: {
      ...
      if (diagIndex == 0) {
        ...
        lcd.setCursor(0,0); lcd.print(F("WDT Resets:"));
        lcd.setCursor(0,1); lcd.print(watchdogResetCount); lcd.print(F(" (Rt: Right clr)"));
      } else if (diagIndex == 6) {
        lcd.setCursor(0,0); lcd.print(F("WDT Crash Test"));
        lcd.setCursor(0,1); lcd.print(F("Press Right ->"));
      } else if (diagIndex == 7) {
    ...
    case CONFIRM_WDT_CRASH: {
      lcd.setCursor(0,0); lcd.print(F("Run WDT Crash?"));
      lcd.setCursor(0,1); lcd.print(cancelChoice ? F("Yes") : F("No "));
      break;
    }
...
void loop() {
  ...
  if (displayActive && millis() - lastAwakeWdtCheck >= 8000) {
    lastAwakeWdtCheck = millis();
    watchdogTick = true;
  }
  ...
  // Combo: Up + Down → toggle servo state or open latch immediately
  if (digitalRead(btnUp) == LOW && digitalRead(btnDown) == LOW ) {
    if (!comboHandled) {
      ...
      if (!pulseActive) activeLockType = determineConnectedLock();

      if (activeLockType == LOCK_MONO_PULSE && !pulseActive && nitinolReady) {
        delay(300);
        retryCount = 0;
        triggerLock();
        menuState = HOME;
  ...
  if (showingMessage && millis() - lastMessageStart >= 2000) {
    ...
    if (menuState == OPTIONS || menuState == CONFIRM_RESET_GATETIMES) {
      menuState = OPTIONS;
    } else {
      menuState = HOME;
    }

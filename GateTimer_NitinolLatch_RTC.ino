...
// Pulse lock output (from V1)
const int lockPin = A2;

// Pulse timing and Voltage drop detection
...
int retryCount = 0;
const int maxRetries = 10;   // or whatever number you want
...
int diagIndex = 0;   // 0=open,1=fail,2=quickest,3=slowest,4=exit
enum PulseOutcome {
  OUT_NONE,
  OUT_OPEN_BREAK,
  ...
  switch (menuState) {
    ...
    case OPTIONS:
      if (!optionsEditMode) {
        ...
        } else if (optionsFieldIndex == 1) {   // pulse duration ms
          pulseDurationMs += 50;
          if (pulseDurationMs > 1000) pulseDurationMs = 50;
        } else if (optionsFieldIndex == 2) {   // Voltage threshold
          voltageThreshold += 5;
    ...
    case OPTIONS:
      if (!optionsEditMode) {
        ...
        if (optionsFieldIndex == 0 ||
            optionsFieldIndex == 1 ||
            optionsFieldIndex == 2 ) {
          // Enter edit mode (Timeout, pulse, volt threshold)
...
void updatePulse() {
  ...
  // Check voltage drop during pulse
  long vccNow = readVcc();
  ...
  // Lock opened early (drop disappeared)
  if (drop < voltageThreshold && !lockOpened) {
    ...
    if (avgDrop < voltageThreshold && maxDrop < voltageThreshold) {
      // Lock opened OR circuit break
      lastOutcome = (retryCount == 0) ? OUT_OPEN_BREAK : OUT_OPEN_BREAK_RETRY;
      lcd.clear();
      lcd.setCursor(0,0);
      lcd.print("Open/Break");
      lcd.setCursor(0,1);
      lcd.print("A");
      lcd.print(dropSamples > 0 ? dropSum / dropSamples : 0);
      lcd.print(" M");
      lcd.print(maxDrop);
      lcd.print(" d");
      lcd.print(drop);
    ...
    if (retryCount > 0){
      ...
      lcd.print(" d");
      lcd.print(drop - voltageThreshold);
    }
    delay(2500);    
    lastMessageStart = millis();
    showingMessage = true;
  ...
  // Pulse finished but lock did NOT open
  if (now - pulseStart >= pulseDurationMs) {
    ...
    if (!lockOpened) {
      ...
      // Increase pulse duration for next attempt
      pulseDurationMs += 100;
      ...
      if (retryCount >= maxRetries) {
        ...
        lcd.clear();
        lcd.setCursor(0,0); lcd.print("Max Retries");
        lcd.setCursor(0,1);
        lcd.print("Fail A:");
        lcd.print(dropSamples > 0 ? dropSum / dropSamples : 0);
        lcd.print(" M:");
      ...
      retryAtRTC = rtc.now() + TimeSpan(0, 0, 0, 15);  // +30 seconds
      retryScheduled = true;
...
void refreshLCD() {
  ...
  switch (menuState) {
    ...
    case OPTIONS: {
      ...
      if (optionsEditMode && (optionsFieldIndex == 0 ||
          optionsFieldIndex == 1 ||
          optionsFieldIndex == 2 )) {
        lcd.setCursor(14,1);
        lcd.print("<>");
    ...
    case DIAGNOSTICS: {
      ...
      if (diagIndex ==0) {
        ...
        lcd.print(" T "); lcd.print(lastSuccessOpenMs); lcd.print("ms");
        ...
        lcd.print(" PF "); lcd.print(lastFailPulseMs); lcd.print("ms");

      } else if (diagIndex == 3) {
        // Quickest Open
        lcd.setCursor(0,0);
        lcd.print("VQ "); lcd.print(bestOpenVcc);
        lcd.print(" AQ "); lcd.print(bestOpenAvg);

        lcd.setCursor(0,1);
        lcd.print("dQ "); lcd.print(bestOpenEnd);
        lcd.print(" TQ "); lcd.print(bestOpenTime); lcd.print("ms");

      } else if (diagIndex == 4) {
        // Slowest Open
        lcd.setCursor(0,0);
        lcd.print("VS "); lcd.print(worstOpenVcc);
        lcd.print(" AS "); lcd.print(worstOpenAvg);

        lcd.setCursor(0,1);
        lcd.print("dS "); lcd.print(worstOpenEnd);
        lcd.print(" TS "); lcd.print(worstOpenTime); lcd.print("ms");
...
// Combo: Up + Down → start a 10 second countdown
if (!pulseActive &&
    digitalRead(btnUp) == LOW &&
    digitalRead(btnDown) == LOW ) {
    
    DateTime now = rtc.now();
    endTime = now + TimeSpan(0, 0, 0, 10);   // 10 seconds from now
    lockActive = true;
    menuState = HOME;
}
